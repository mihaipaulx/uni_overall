keywords = {
  "calendar": {
    "query": "academic calendar",
    "url": ["calendar", "intake", "term"],
  },
  "programs": {
    "query": "academic programs",
    "url": ["program", "degree", "course", "major", "academics"],
  },
  "tuition": {
    "query": "tuition fees",
    "url": ["tuition", "cost", "fees", "finaid", "financial"],
  },
  "international_tuition": {
    "query": "international tuition fees",
    "url": ["tuition", "cost", "fees", "finaid", "financial"],
  },
  "deadlines": {
    "query": "application deadlines",
    "url": ["apply", "deadline", "application", "admission"],
  },
  "international_deadlines": {
    "query": "international application deadlines",
    "url": ["apply", "deadline", "application", "admission"],
  },
  "requirements": {
    "query": "admission requirements",
    "url": ["requirement", "document"],
  },
  "english": {
    "query": "english requirements",
    "url": [""],
  },
  "graduate_programs": {
    "query": "graduate academic programs",
    "url": ["program", "degree", "course", "major", "academics"],
  },
  "graduate_tuition": {
    "query": "graduate tuition fees",
    "url": ["tuition", "cost", "fees", "finaid", "financial"],
  },
  "international_graduate_tuition": {
    "query": "international graduate tuition fees",
    "url": ["tuition", "cost", "fees", "finaid", "financial"],
  },
  "graduate_deadlines": {
    "query": "graduate application deadlines",
    "url": ["apply", "deadline", "application", "admission"],
  },
  "international_graduate_deadlines": {
    "query": "international graduate application deadlines",
    "url": ["apply", "deadline", "application", "admission"],
  },
  "graduate_requirements": {
    "query": "graduate admission requirements",
    "url": ["requirement", "document"],
  },
  "graduate_english": {
    "query": "graduate english requirements",
    "url": [""],
  },
}